#ifndef STRASSEN_CUBLAS_H
#define STRASSEN_CUBLAS_H
...
constexpr int STRASSEN_CUBLAS_THRESHOLD = 64;

class StrassenCuBLASContext {
public:
    cublasHandle_t handle;
    
    StrassenCuBLASContext() {
        cublasCreate(&handle);
    }
    ...
    void gemm_cublas_device(const float* d_A, const float* d_B, float* d_C, int N) {
...
void strassen_cublas_impl(StrassenCuBLASContext& ctx, const float* d_A, const float* d_B, float* d_C, int N, int threshold);

void strassen_cublas_impl(StrassenCuBLASContext& ctx, const float* d_A, const float* d_B, float* d_C, int N, int threshold) {
    if (N <= threshold || N % 2 != 0) {
        ctx.gemm_cublas_device(d_A, d_B, d_C, N);
        return;
    }
    ...
    strassen_cublas_impl(ctx, d_T1, d_T2, d_M1, mid, threshold);
    
    ctx.matrix_add_device(d_A21, d_A22, d_T1, mid);
    strassen_cublas_impl(ctx, d_T1, d_B11, d_M2, mid, threshold);
    
    ctx.matrix_sub_device(d_B12, d_B22, d_T1, mid);
    strassen_cublas_impl(ctx, d_A11, d_T1, d_M3, mid, threshold);
    
    ctx.matrix_sub_device(d_B21, d_B11, d_T1, mid);
    strassen_cublas_impl(ctx, d_A22, d_T1, d_M4, mid, threshold);
    
    ctx.matrix_add_device(d_A11, d_A12, d_T1, mid);
    strassen_cublas_impl(ctx, d_T1, d_B22, d_M5, mid, threshold);
    
    ctx.matrix_sub_device(d_A21, d_A11, d_T1, mid);
    ctx.matrix_add_device(d_B11, d_B12, d_T2, mid);
    strassen_cublas_impl(ctx, d_T1, d_T2, d_M6, mid, threshold);
    
    ctx.matrix_sub_device(d_A12, d_A22, d_T1, mid);
    ctx.matrix_add_device(d_B21, d_B22, d_T2, mid);
    strassen_cublas_impl(ctx, d_T1, d_T2, d_M7, mid, threshold);
...
static int g_strassen_cublas_threshold = STRASSEN_CUBLAS_THRESHOLD;

void gemm_strassen_cublas_wrapper(const float* A, const float* B, float* C, int N) {
    StrassenCuBLASContext ctx;
    ...
    strassen_cublas_impl(ctx, d_A, d_B, d_C, N, g_strassen_cublas_threshold);
