#ifndef WINOGRAD_CUBLAS_H
#define WINOGRAD_CUBLAS_H
...
constexpr int WINOGRAD_CUBLAS_THRESHOLD = 64;

class WinogradCuBLASContext {
public:
    cublasHandle_t handle;
    
    WinogradCuBLASContext() {
        cublasCreate(&handle);
    }
...
void winograd_cublas_impl(WinogradCuBLASContext& ctx, const float* d_A, const float* d_B, float* d_C, int N, int threshold);
...
void winograd_cublas_impl(WinogradCuBLASContext& ctx, const float* d_A, const float* d_B, float* d_C, int N, int threshold) {
    ...
    winograd_cublas_impl(ctx, d_T1, d_T2, d_M1, mid, threshold);
    
    ctx.matrix_add_device(d_A21, d_A22, d_T1, mid);
    winograd_cublas_impl(ctx, d_T1, d_B11, d_M2, mid, threshold);
    
    ctx.matrix_sub_device(d_B12, d_B22, d_T1, mid);
    winograd_cublas_impl(ctx, d_A11, d_T1, d_M3, mid, threshold);
    
    ctx.matrix_sub_device(d_B21, d_B11, d_T1, mid);
    winograd_cublas_impl(ctx, d_A22, d_T1, d_M4, mid, threshold);
    
    ctx.matrix_add_device(d_A11, d_A12, d_T1, mid);
    winograd_cublas_impl(ctx, d_T1, d_B22, d_M5, mid, threshold);
    
    ctx.matrix_sub_device(d_A21, d_A11, d_T1, mid);
    ctx.matrix_add_device(d_B11, d_B12, d_T2, mid);
    winograd_cublas_impl(ctx, d_T1, d_T2, d_M6, mid, threshold);
    
    ctx.matrix_sub_device(d_A12, d_A22, d_T1, mid);
    ctx.matrix_add_device(d_B21, d_B22, d_T2, mid);
    winograd_cublas_impl(ctx, d_T1, d_T2, d_M7, mid, threshold);
...
static int g_winograd_cublas_threshold = WINOGRAD_CUBLAS_THRESHOLD;

void gemm_winograd_cublas_wrapper(const float* A, const float* B, float* C, int N) {
    WinogradCuBLASContext ctx;
    ...
    winograd_cublas_impl(ctx, d_A, d_B, d_C, N, g_winograd_cublas_threshold);
