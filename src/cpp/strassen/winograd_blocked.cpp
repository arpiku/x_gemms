#ifndef WINOGRAD_BLOCKED_H
#define WINOGRAD_BLOCKED_H
...
constexpr size_t WINOGRAD_THRESHOLD_BLOCKED = 64;
constexpr size_t WINOGRAD_BLOCK_SIZE = 64;

template <typename T>
void winograd_gemm_blocked_base(const T* A, const T* B, T* C, size_t N, size_t block_size = WINOGRAD_BLOCK_SIZE) {
...
template <typename T>
void winograd_blocked_impl(const T* A, const T* B, T* C, size_t N, size_t threshold, size_t block_size) {
    if (N <= threshold || N % 2 != 0) {
        winograd_gemm_blocked_base(A, B, C, N, block_size);
        return;
    }
    ...
    winograd_blocked_impl(T1, T2, M1, mid, threshold, block_size);
    
    strassen_add_matrix(A21, A22, T1, mid);
    winograd_blocked_impl(T1, B11, M2, mid, threshold, block_size);
    
    strassen_sub_matrix(B12, B22, T1, mid);
    winograd_blocked_impl(A11, T1, M3, mid, threshold, block_size);
    
    strassen_sub_matrix(B21, B11, T1, mid);
    winograd_blocked_impl(A22, T1, M4, mid, threshold, block_size);
    
    strassen_add_matrix(A11, A12, T1, mid);
    winograd_blocked_impl(T1, B22, M5, mid, threshold, block_size);
    
    strassen_sub_matrix(A21, A11, T1, mid);
    strassen_add_matrix(B11, B12, T2, mid);
    winograd_blocked_impl(T1, T2, M6, mid, threshold, block_size);
    
    strassen_sub_matrix(A12, A22, T1, mid);
    strassen_add_matrix(B21, B22, T2, mid);
    winograd_blocked_impl(T1, T2, M7, mid, threshold, block_size);
...
template <typename T>
void gemm_winograd_blocked(const T* A, const T* B, T* C, size_t N, size_t threshold = WINOGRAD_THRESHOLD_BLOCKED, size_t block_size = WINOGRAD_BLOCK_SIZE) {
    std::memset(C, 0, N * N * sizeof(T));
    winograd_blocked_impl(A, B, C, N, threshold, block_size);
}

template void gemm_winograd_blocked<float>(const float*, const float*, float*, size_t, size_t, size_t);
template void gemm_winograd_blocked<double>(const double*, const double*, double*, size_t, size_t, size_t);
