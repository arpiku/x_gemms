#ifndef STRASSEN_NAIVE_H
#define STRASSEN_NAIVE_H
...
template <typename T>
void gemm_naive_base(const T* A, const T* B, T* C, size_t N) {
...
template <typename T>
void strassen_naive_impl(const T* A, const T* B, T* C, size_t N, size_t threshold) {
    if (N <= threshold) {
        gemm_naive_base(A, B, C, N);
        return;
    }
    ...
    strassen_naive_impl(T1, T2, M1, mid, threshold);
    
    strassen_add_matrix(A21, A22, T1, mid);
    strassen_naive_impl(T1, B11, M2, mid, threshold);
    
    strassen_sub_matrix(B12, B22, T1, mid);
    strassen_naive_impl(A11, T1, M3, mid, threshold);
    
    strassen_sub_matrix(B21, B11, T1, mid);
    strassen_naive_impl(A22, T1, M4, mid, threshold);
    
    strassen_add_matrix(A11, A12, T1, mid);
    strassen_naive_impl(T1, B22, M5, mid, threshold);
    
    strassen_sub_matrix(A21, A11, T1, mid);
    strassen_add_matrix(B11, B12, T2, mid);
    strassen_naive_impl(T1, T2, M6, mid, threshold);
    
    strassen_sub_matrix(A12, A22, T1, mid);
    strassen_add_matrix(B21, B22, T2, mid);
    strassen_naive_impl(T1, T2, M7, mid, threshold);
...
template <typename T>
void gemm_strassen_naive(const T* A, const T* B, T* C, size_t N, size_t threshold = STRASSEN_THRESHOLD) {
    std::memset(C, 0, N * N * sizeof(T));
    strassen_naive_impl(A, B, C, N, threshold);
}

template void gemm_strassen_naive<float>(const float*, const float*, float*, size_t, size_t);
template void gemm_strassen_naive<double>(const double*, const double*, double*, size_t, size_t);
