#ifndef WINOGRAD_CACHE_AWARE_H
#define WINOGRAD_CACHE_AWARE_H
...
constexpr size_t WINOGRAD_THRESHOLD_CACHE = 64;
constexpr size_t WINOGRAD_CACHE_BLOCK_SIZE = 32;

template <typename T>
class WinogradMemoryPool {
private:
    T* pool;
    size_t capacity;
    size_t offset;
...
public:
    WinogradMemoryPool(size_t max_n) {
        size_t total_elements = 0;
        size_t n = max_n;
        while (n > WINOGRAD_THRESHOLD_CACHE && n % 2 == 0) {
            size_t sub_n = n / 2;
...
template <typename T>
void winograd_cache_aware_base(const T* A, const T* B, T* C, size_t N, size_t block_size = WINOGRAD_CACHE_BLOCK_SIZE) {
...
template <typename T>
void winograd_cache_aware_impl(const T* A, const T* B, T* C, size_t N, 
                                size_t threshold, size_t block_size,
                                WinogradMemoryPool<T>& pool) {
    if (N <= threshold || N % 2 != 0) {
        winograd_cache_aware_base(A, B, C, N, block_size);
        return;
    }
    ...
    winograd_cache_aware_impl(T1, T2, M1, mid, threshold, block_size, pool);
    pool.set_offset(saved_offset);
    
    strassen_add_matrix(A21, A22, T1, mid);
    winograd_cache_aware_impl(T1, B11, M2, mid, threshold, block_size, pool);
    pool.set_offset(saved_offset);
    
    strassen_sub_matrix(B12, B22, T1, mid);
    winograd_cache_aware_impl(A11, T1, M3, mid, threshold, block_size, pool);
    pool.set_offset(saved_offset);
    
    strassen_sub_matrix(B21, B11, T1, mid);
    winograd_cache_aware_impl(A22, T1, M4, mid, threshold, block_size, pool);
    pool.set_offset(saved_offset);
    
    strassen_add_matrix(A11, A12, T1, mid);
    winograd_cache_aware_impl(T1, B22, M5, mid, threshold, block_size, pool);
    pool.set_offset(saved_offset);
    
    strassen_sub_matrix(A21, A11, T1, mid);
    strassen_add_matrix(B11, B12, T2, mid);
    winograd_cache_aware_impl(T1, T2, M6, mid, threshold, block_size, pool);
    pool.set_offset(saved_offset);
    
    strassen_sub_matrix(A12, A22, T1, mid);
    strassen_add_matrix(B21, B22, T2, mid);
    winograd_cache_aware_impl(T1, T2, M7, mid, threshold, block_size, pool);
    pool.set_offset(saved_offset);
...
template <typename T>
void gemm_winograd_cache_aware(const T* A, const T* B, T* C, size_t N, 
                                size_t threshold = WINOGRAD_THRESHOLD_CACHE, 
                                size_t block_size = WINOGRAD_CACHE_BLOCK_SIZE) {
    std::memset(C, 0, N * N * sizeof(T));
    WinogradMemoryPool<T> pool(N);
    winograd_cache_aware_impl(A, B, C, N, threshold, block_size, pool);
}

template void gemm_winograd_cache_aware<float>(const float*, const float*, float*, size_t, size_t, size_t);
template void gemm_winograd_cache_aware<double>(const double*, const double*, double*, size_t, size_t, size_t);
